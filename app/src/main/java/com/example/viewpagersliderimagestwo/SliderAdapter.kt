...
class SliderAdapter(
    ...
    private val pageTitles = fragment.resources.getStringArray(R.array.fragmentTitles)
    private val drawableList = getResourceIdArray(fragment.resources.obtainTypedArray(R.array.drawableList))
    private val totalPagesCount = pageTitles.size

    override fun getItemCount() = totalPagesCount
    override fun createFragment(position: Int): Fragment {
        val data = FragmentData(pageTitles[position], drawableList[position])
        return MainPageFragment.newInstance(data)
    }

    private fun getResourceIdArray(typedArray: TypedArray): Array<Int> {
